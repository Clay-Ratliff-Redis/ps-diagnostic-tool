...
class Nodes(BaseCheckSuite):
    ...
    def _get_quorum_only_nodes(self):
        ...
        rsps = [self.rex().exec_uni(f'{install_dir}/bin/rladmin info node {node["uid"]}',
                                    self.rex().get_targets()[0], True) for node in nodes]
    ...
    def _get_file_systems(self, _path):
        ...
        """
        rsps = self.rex().exec_broad(f'df {_path}', True)
    ...
    def check_nodes_config_001(self, _params):
        ...
        """
        install_dir = self.get_remote_env_var('installdir')
        rsps = self.rex().exec_broad(f'{install_dir}/bin/rlcheck', True)
    ...
    def check_nodes_config_002(self, _params):
        ...
        Executes `df $logdir` and compares the mountpoint to '/'.

        Remedy: Move log file path to a mounted filesystem.

        :param _params: None
        :returns: result
        """
        api = True  # API is called in subroutine
        log_dir = self.get_remote_env_var('logdir')
        mounts = self._get_file_systems(log_dir)
    ...
    def check_nodes_config_003(self, _params):
        ...
        Remedy: Move ephemeral storage path to a mounted filesystem.
    ...
    def check_nodes_config_004(self, _params):
        ...
        Remedy: Move persistent storage path to a mounted filesystem.
    ...
    def check_nodes_config_009(self, _params):
        ...
        Executes `grep error $logdir/install.log` and counts result.
        ...
        """
        log_dir = self.get_remote_env_var('logdir')
        rsps = self.rex().exec_broad(f'grep error {log_dir}/install.log || echo ""')
    ...
    def check_nodes_config_011(self, _params):
        ...
        bin_dir = self.get_remote_env_var('bindir')
        python_bin = f'{bin_dir}/python'
        cmd = python_bin + ''' -c "import socket; socket.create_connection(('"'{0}'"', {1}))" 2> /dev/null || echo '"'{0}:{1}'"' '''
    ...
    def check_nodes_status_001(self, _params):
        ...
        """
        install_dir = self.get_remote_env_var('installdir')
        rsps = self.rex().exec_broad(f'{install_dir}/bin/cnm_ctl status', True)
    ...
    def check_nodes_status_002(self, _params):
        ...
        Remedy: Investigate node with `supervisorctl status`.

        :param _params: None
        :returns: result
        """
        install_dir = self.get_remote_env_var('installdir')
        rsps = self.rex().exec_broad(f'{install_dir}/bin/supervisorctl status', True)
        not_running = [(re.findall(r'^((?!RUNNING).)*$', rsp.result(), re.MULTILINE), rsp.target) for rsp in rsps]
        sum_not_running = sum([len(r[0]) for r in not_running])

        return sum_not_running == 0, {
            f'node:{self.api().get_uid(self.rex().get_addr(r[1]))}': len(r[0]) for r in not_running}
